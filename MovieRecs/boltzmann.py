...
class RBM():
    def __init__(self, visible_nodes_count, hidden_nodes_count):
        ...
        def get_visible_sample(self, hidden_neurons):
            weight_x_neurons = torch.mm(hidden_neurons, self.weights)
        ...
        def train(self, ratings_per_user, ratings_per_user_k, prob_hidden, prob_hidden_k):
            self.weights += torch.mm(ratings_per_user.t(), prob_hidden) - torch.mm(ratings_per_user_k.t(), prob_hidden_k)
            self.visible_bias += torch.sum((ratings_per_user - ratings_per_user_k), 0)
            self.hidden_bias += torch.sum((prob_hidden - prob_hidden_k), 0)
